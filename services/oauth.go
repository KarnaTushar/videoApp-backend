...
func (router *ServiceRouter) Handler(w http.ResponseWriter, r *http.Request) (*string, *string, *string, error) {
	...
	err = router.DB.Get(&userData, "SELECT id, identifier, user_name, email FROM users WHERE email=?", userInfo.Email)
...
func (r *ServiceRouter) GetUserInfo(oauthConfig oauth2.Config, oauthDetails Details, provider *oidc.Provider) (*User, error) {
	...
	err := r.DB.Get(&tokenData, "SELECT id, code, access_token, refresh_token, token_type, expiry FROM credentials WHERE code=?", oauthDetails.Code)
	if err != nil {
