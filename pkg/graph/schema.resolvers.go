...
func (r *mutationResolver) MutePstn(ctx context.Context, uid int, passphrase string, mute *bool) (*models.UIDMuteState, error) {
	...
	err := r.DB.Get(&channelData, "SELECT title, channel_name, channel_secret, host_passphrase, viewer_passphrase, dtmf FROM channels WHERE host_passphrase = ? OR viewer_passphrase = ?", passphrase, passphrase)
	if err != nil {
...
func (r *mutationResolver) SetPresenter(ctx context.Context, uid int, passphrase string) (int, error) {
	...
	err := r.DB.Get(&channelData, "SELECT id, title, channel_name, channel_secret, host_passphrase, viewer_passphrase, recording_rid, recording_sid, recording_uid FROM channels WHERE host_passphrase = ? OR viewer_passphrase = ?", passphrase, passphrase)
	if err != nil {
		r.Logger.Error().Err(err).Str("passphrase", passphrase).Msg("Invalid Passphrase")
...
func (r *mutationResolver) StartRecordingSession(ctx context.Context, passphrase string, secret *string) (string, error) {
	...
	err = r.DB.Get(&channelData, "SELECT id, title, channel_name, channel_secret, host_passphrase, viewer_passphrase FROM channels WHERE host_passphrase = ? OR viewer_passphrase = ?", passphrase, passphrase)
	if err != nil {
		r.Logger.Error().Err(err).Str("passphrase", passphrase).Msg("Invalid Passphrase")
		return "", errors.New("Invalid URL")
	}
...
func (r *mutationResolver) LogoutSession(ctx context.Context, token string) ([]string, error) {
	...
	err = r.DB.Select(&tokens, "SELECT * FROM tokens WHERE user_id = ?", authUser.ID)
	if err != nil {
...
func (r *queryResolver) JoinChannel(ctx context.Context, passphrase string) (*models.Session, error) {
	...
	err := r.DB.Get(&channelData, "SELECT title, channel_name, channel_secret, host_passphrase, viewer_passphrase FROM channels WHERE host_passphrase = ? OR viewer_passphrase = ?", passphrase, passphrase)
	if err != nil {
		r.Logger.Error().Err(err).Str("passphrase", passphrase).Msg("Invalid Passphrase")
		return nil, errors.New("Invalid URL")
	}
